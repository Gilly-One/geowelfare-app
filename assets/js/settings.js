(async function(){
  ...
  function openAnnouncement(a){ const mod=GWF.components.modal(a?'Edit announcement':'Add announcement', announcementForm(a)); mod.querySelector('#announcementForm').onsubmit=async e=>{ e.preventDefault(); const data=Object.fromEntries(new FormData(e.target).entries()); data.active = data.active === 'true'; if(a) await GWF.db.update('announcements', a.id, data); else await GWF.db.add('announcements', data); if(data.active) GWF.pwa?.sendPush?.({targetRole:'member', title:'New welfare announcement', body:data.title || 'New announcement posted', url:'/member-portal.html', tag:'announcement'}); announcements=await GWF.db.all('announcements'); mod.remove(); renderAnnouncements(); GWF.components.renderSidebar?.(); GWF.toast('Announcement saved.','success'); }; }
